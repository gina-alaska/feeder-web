class HighlightsController < ApplicationController
  ...
  def create
    @highlight = @entry.build_highlight(highlight_params)
    @highlight.user = current_user
    
    respond_to do |format|
      if @highlight.save
        format.html { redirect_to @entry, notice: 'Highlight was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @highlight.update(highlight_params)
        format.html { redirect_to @entry, notice: 'Highlight was successfully updated.' }
  ...
  def destroy
    @highlight.destroy
    respond_to do |format|
      format.html { redirect_to @entry, notice: 'Highlight was successfully destroyed.' }
  ...
  private
    ...
    def set_highlight
      @highlight = @entry.highlight unless @entry.nil?
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def highlight_params
      params.require(:highlight).permit(:user_id, :description)
    end
end
